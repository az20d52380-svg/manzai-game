...
func buildFinalsCast(year: Int, runSeed: UInt64,
    ...
    // --- (3) champion が谷の年ならモブ（TOP3 rival と被らせない） ---
    let excludeForChamp = Set([playerName] + reservedRivals)
    if champion.isEmpty {
        champion = drawMobs(runSeed, year: year, count: 1,
                            exclude: excludeForChamp,
                            poolFilter: { CHAMP_ELIGIBLE.contains($0) }).first!
        // flavor は既に _generic
    }

    // --- (4) TOP3 他2枠 = rival候補 + モブ補充（2 になるまで） ---
    let need = 2 - reservedRivals.count
    let excludeForFill = Set([playerName, champion] + reservedRivals)
    let fill = drawMobs(runSeed, year: year, count: need,
                        exclude: excludeForFill,
                        poolFilter: { _ in true })
    // 注: champion のモブ抽選と fill の抽選は同じ year 系列だが Salt.mobDrap の
    //     連番 i を champion=0 起点で通し、fill 側は i を継続させて重複ゼロを保証する
    //     （実装では drawMobs を 1 回で必要総数ぶん引き、先頭を champion に割る方式でも可）
...
> 実装上の要点: **champion モブと TOP3 補充モブの非重複**を厳密にしたいので、推奨実装は「その年に必要なモブ総数（谷の年なら最大 1+2=3、優勝周回なら 0+2=2）を **drawMobs で一度に引き**、先頭を champion、残りを fill に割る」方式。上記は読みやすさ優先で 2 回に割っているが、その場合は連番 `i` を跨いで共有すること（同じ index を二度引かないため）。
...
```json
{
  "year": 9,
  "runSeed": 20260704,
  "champion": { "name": "静物画", "flavor": "あの静物画が、ついに。8年分の準決勝を、今夜ぜんぶ回収した", "isRival": true },
  "top3Others": [
    { "name": "静物画", "isRival": true },
    { "name": "缶コーヒー兄弟", "isRival": false }
  ]
}
```

※ 上例は説明用。実際には champion と top3Others に**同一組は入らない**（§4 の exclude 制御）。9年目の正しい出力は champion=モブ（谷の年）、top3Others=[静物画, モブ] となる。
