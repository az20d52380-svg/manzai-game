...
- **育成UIリデザイン**: `ui_redesign_pawapuro_uma_v0.md`（パワプロ サクセス/ウマ娘を調査→現状gapの正体→再設計スペック＝北極星「選ぶ→ゴースト予告→大実行ボタン→段階リビール→次週」／**RNG非消費の正直getter**定義（previewGains/baseScore/nextStageLine/injuryRisk）＝素朴版は逓減無視で嘘つき注意／P0優先順＝①段階リビール②地力vs通過ラインゲージ③ゴースト予告／critiqueの構造的弱点＝友情トレ不在・やる気非機構）。**オーナー確定v7＝パワプロ2022式トップメニュー：①最初は稽古/回復/バイト(＋データ/アイテム)に分かれたコマンド→②稽古を押すと変種(ネタ作り等)が出る→③変種を押したらそのまま実行(決定ボタン無し・押す＝実行／画面隅の"決定"はコントローラ凡例でUIボタンではない)。効果は各カードに常時表示(正直な+増加分)④能力アップ/おでかけ/システム(セーブ)は無し＝セーブ自動⑤データ(実績)/アイテムの器は残す⑥怪我率(体調ダウン率)・稽古Lvは廃止・実装しない⑦能力は左上ピル/体力・所持金・週は下の帯/通過ライン非表示/右のサポカ無し**。見た目正典モック `ui_mockup_pawapuro_v7.html`（**Playwright検証済＝実際に動く**・タップ可／v3-v6は旧・履歴）。**教訓: モックは公開前に /opt/pw-browsers-1194 のchromiumでロードしconsole/pageerror無し＋押下で状態遷移まで検証する(v5は親innerHTML上書きで子要素消え全render throw→無反応だった)**
