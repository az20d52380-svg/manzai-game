...
| medium | ハマった夜の「実測」値が別ライン(76/82)で取得され、決勝到達率が43.5%と29%で二重掲載 | 整合 | canonical §2表(43.5%/74・80) ⇔ §4 L95(29%/76・82) / finals17-28% / master 2-4割 / red_team 22% | 4.8-cloud | §4を「(76/82旧測定・アンカーは74/80へ移動)」注記し74/80での到達を§2に一本化。他文書の%表現をポインタ置換 |
| medium | finals §5未決2「点差非表示」とchampion_selection「優勝者との差Δを表示」が半矛盾(下位が先行確定) | 整合 | finals §5未決2 L152 ⇔ finals §4-D L142 / champion_selection §7 L285 | owner-decision | 「相対Δのみ表示・絶対点/ラインは非表示」で確定ならfinals §5未決2を✅解消へ更新。逆算誘発の許容可否をオーナー1行確認 |
