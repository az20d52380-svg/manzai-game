...
**オーナー決定（2026-07-06）**：「分岐を拡張する」——`else`（それ以外＝決勝未到達・非破産）分岐の**中をさらに二分**する。

- **実装方針**: `YearResultView.monolog`の`else`分岐に、**知名度(`fame`)または到達回戦数(`roundsPassed`)を見る決定的if判定を1本追加**する（新規の乱数ロールなし＝golden不変。GameCore/数式には触れず、UI層の分岐条件を1つ増やすだけ）。
  ```swift
  // 【提案・未適用】else分岐の内部拡張イメージ
  if o.champion { ... }
  if o.bankrupt { return /* 貧乏年 content（corpus yearEnd.貧乏） */ }
  if o.reachedFinal { ... }
  if o.roundsPassed >= 3 || s.fame >= 30 { return /* 躍進年 content（corpus yearEnd.躍進） */ }  // 【仮】閾値
  return /* 停滞年 content（corpus yearEnd.停滞） */
  ```
  閾値（`roundsPassed >= 3`／`fame >= 30`）は**完全に【仮】**——CLI側で`sim_career.py`を回し、実際のプレイ分布（1年目でどこまで到達するのが「良い年」に見えるか）を見てから確定すること。
- **対応づけ**: `bankrupt`→corpus `yearEnd.貧乏`、`else`拡張の上枝→`yearEnd.躍進`、`else`拡張の下枝→`yearEnd.停滞`。谷口の台詞部分は`会話.決勝進出`/`会話.準決敗退`等、Task6で書き直し済みの行に差し替え可。
- **`9年目未優勝`は依然、本編（多年）機能として保留**——`GameSession`が`year=1`固定である限り到達不可能なので、無理に当てはめない。多年キャリア実装時に別途配線。
...
### A-5-補. 破産（夜逃げ）ライン `bankruptLine = -1,000,000` — オーナー決定：sim実測待ち

オーナーから「-100万円は多い気がする」という懸念が出た（下積みでその額の借金は重すぎる）。**オーナー決定：今は数値を変えず、sim実測に任せて後で確定する。**

- **この場では数値を変更しない**（バランス数値の変更は`tools/*.py`正典→GameCore→`gen_golden.py`再生成→`swift test`緑を1コミットで揃える必要があり、proposals専任セッションの管轄外）。
- **CLI側でやること**: `tools/sim_career.py`で`bankruptLine`を`-300,000`〜`-1,000,000`の範囲で振って、実際に何%のプレイが夜逃げするかを実測する。「下積みらしい緊張感が出るが理不尽ではない」帯を探してから、オーナーに実測値を見せて確定する。
- 数値を変更する場合は、CLAUDE.md §B「バランス数値は`sim_career.py`の実測値を根拠にコミットへ添える」を厳守。
